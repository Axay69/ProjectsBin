...
function persist(
  state: Pick<NotesState, 'notes' | 'darkMode' | 'colorMode' | 'fixedColor'>,
) {
  try {
...
export const useNotesStore = create<NotesState>()((set, get) => ({
  ...
  add: ({ title, content, tags, color, media, pinned }) => {
    const resolvedColor =
      color ?? pickNoteColor(get().colorMode, get().fixedColor);
    ...
    persist({
      ...
      fixedColor: get().fixedColor,
    });
    return note;
  },
  update: (id, patch) => {
    const normalizedMedia = normalizeMedia(
      patch.media as MediaItem[] | undefined,
    );
    const next = get().notes.map(n => {
      ...
      return {
        ...
        media: mediaToUse,
        thumbnailUri:
          patch.thumbnailUri ?? thumbFromMedia(mediaToUse) ?? n.thumbnailUri,
        updatedAt: Date.now(),
      };
    });
    set({ notes: next });
    ...
    if (raw) {
      try {
        const parsed = JSON.parse(raw) as Partial<
          Pick<NotesState, 'notes' | 'darkMode' | 'colorMode' | 'fixedColor'>
        >;
        const normalizedNotes = (parsed.notes ?? []).map(n => ({
          ...n,
          color:
            n.color ??
            pickNoteColor(parsed.colorMode ?? 'random', parsed.fixedColor),
        }));
        set({
