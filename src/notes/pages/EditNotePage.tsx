...
export default function EditNotePage({ route, navigation }: Props) {
  ...
  const [color, setColor] = useState(
    existing?.color ?? pickNoteColor(colorMode, fixedColor),
  );
  ...
  const save = async () => {
    if (!title.trim() && !content.trim()) {
      Alert.alert(
        'Add a title or text',
        'Please add at least a title or some content before saving.',
      );
      return;
    }
    setSaving(true);
  ...
  const pickMedia = async (type: 'photo' | 'video') => {
    ...
    try {
      ...
      const item: MediaItem = {
        id: `${Date.now()}_${mediaType}`,
        type: mediaType,
        uri,
        name: asset.fileName ?? mediaType,
      };
      setAttachments(prev => [...prev, item]);
    } catch (e) {
      Alert.alert(
        'Unable to attach',
        'Please try again after granting storage permission.',
      );
    }
  };
  ...
  const recordAudio = async () => {
    ...
    try {
      ...
      if (recording) {
        ...
        Alert.alert(
          'Recording',
          'Recording started. Tap "Stop Recording" to finish.',
          [
            {
              text: 'Stop Recording',
              onPress: async () => {
      ...
      Alert.alert(
        'Error',
        `Failed to record audio: ${error.message || 'Unknown error'}`,
      );
      setRecording(false);
    }
  };
    ...
    return (
      ...
      >
        <View style={styles.attachmentRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text
              style={[styles.attachmentTitle, { color: theme.text }]}
              numberOfLines={1}
            >
              {item.name ?? item.type}
            </Text>
            <Text style={[styles.attachmentMeta, { color: theme.subtext }]}>
          ...
          <TouchableOpacity
            ...
            style={[styles.removeBtn, { backgroundColor: theme.muted }]}
          >
  ...
  return (
    <NoteWrapper noteColor={color} darkMode={darkMode}>
      <SafeAreaView style={styles.container} edges={['top']}>
          ...
          >
            ...
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.sectionHint, { color: theme.subtext }]}>
                Tap to pick, long-press to set default
              </Text>
            </View>
            {paletteRow}
            ...
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.sectionHint, { color: theme.subtext }]}>
                Add media and remove before saving
              </Text>
            </View>
            <View style={styles.attachRow}>
              <TouchableOpacity
                ...
                style={[styles.attachBtn, { backgroundColor: theme.primary }]}
              >
        ...
        <SafeAreaView
          edges={['bottom']}
          style={[styles.footerContainer, { borderTopColor: theme.border }]}
        >
          <TouchableOpacity
            activeOpacity={0.7}
        ...
        >
          ...
          >
            ...
            >
              <Text
                style={[styles.actionSheetOptionText, { color: theme.text }]}
              >
                📁 Pick from Library
              </Text>
            </TouchableOpacity>
