...
export default function Index({ navigation }: Props) {
  ...
  const [filter, setFilter] = useState<'all' | 'image' | 'audio' | 'sketch' | 'video'>('all')

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    return notes.filter(n => {
      const matchesQuery = !q || n.title.toLowerCase().includes(q) || n.content.toLowerCase().includes(q)
      const matchesFilter = filter === 'all' ? true : (n.media ?? []).some(m => m.type === filter)
  ...
  const renderItem = ({ item }: { item: Note }) => (
    <NoteCard
      ...
      theme={theme}
      filter={filter}
      onPress={() => navigation.navigate({ name: 'NoteDetail', params: { id: item.id } })}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      <View style={styles.headerBar}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, { color: theme.text }]}>Field Notes</Text>
          <Text style={[styles.subheader, { color: theme.subtext }]}>Capture thoughts, media, and sketches</Text>
        </View>
      </View>
      ...
      <FilterPills pills={pills} active={filter} onChange={key => setFilter(key as any)} theme={theme} />
      <FlatList
        data={filtered}
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptySubtitle, { color: theme.subtext }]}>Create your first note with the plus button.</Text>
          </View>
        }
      />
      <QuickAddFab onPress={() => navigation.navigate({ name: 'EditNote', params: { id: undefined } })} />
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  headerBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 12 },
  logo: { fontSize: 22 },
  ...
  input: { flex: 1, borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, height: 46 },
