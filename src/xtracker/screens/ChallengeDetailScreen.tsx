...
export default function ChallengeDetailScreen({
  ...
  const [challenge, setChallenge] = ((): [any, (c: any) => void] => {
    ...
    const set = (c: any) => {
      /* noop placeholder for static state hook replacement */
    };
    return [initial, set];
  })();
  ...
  const completedDays = baseChallenge.days.filter(
    d => d.status === 'done',
  ).length;
  const currentStreak = baseChallenge.days.filter(
    d => d.status === 'done',
  ).length;
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={[styles.header, { backgroundColor: bgColor }]}>
          ...
          <Text style={[styles.subtitle, { color: subTextColor }]}>
            {baseChallenge.duration} day challenge
          </Text>
        </View>
        ...
        <View style={styles.content}>
          <View style={[styles.progressCard, { backgroundColor: bgColor }]}>
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.stat}>
                <Text style={[styles.statValue, { color: textColor }]}>
                  {baseChallenge.days.filter(d => d.status === 'failed').length}
                </Text>
                <Text style={[styles.statLabel, { color: subTextColor }]}>
                  Misses
                </Text>
          ...
          <View style={styles.section}>
            ...
            <CalendarGrid
              days={baseChallenge.days}
              onDayPress={handleDayUpdate}
              darkMode={darkMode}
            />
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.actionsGrid}>
              ...
              <TouchableOpacity
                style={[styles.actionButton, { backgroundColor: t.error }]}
              >
                <MaterialCommunityIcons
                  name="close"
                  size={24}
