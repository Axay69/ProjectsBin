...
export default function FriendProgressScreen({
  ...
  if (!friend) {
    return (
      <View
        style={[
          styles.container,
          { backgroundColor: darkMode ? '#0F1419' : '#F9FAFB' },
        ]}
      >
        <Text style={{ color: darkMode ? '#F3F4F6' : '#1F2937' }}>
          Friend not found
        </Text>
      </View>
    );
  ...
  const sharedChallenges = seedChallenges.filter(
    c =>
      c.sharedWith.includes(friend.id) ||
      friend.mutualChallenges.includes(c.id),
  );
          ...
          {sharedChallenges.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptySubtitle, { color: subTextColor }]}>
                You haven't started any challenges together yet.
              </Text>
            </View>
          ) : (
            sharedChallenges.map(challenge => {
              const completed = challenge.days.filter(
                d => d.status === 'done',
              ).length;
              return (
                <View
                ...
                >
                  ...
                  <View style={styles.progressComparison}>
                    ...
                    <View style={styles.progressColumn}>
                      ...
                      <ProgressRing
                        current={Math.floor(
                          Math.random() * challenge.duration * 0.8,
                        )}
                        total={challenge.duration}
                        size={100}
                        primaryColor="#06B6D4"
