        ...
        <View style={styles.content}>
          {friends.length === 0 ? (
            <View style={styles.emptyState}>
              <MaterialCommunityIcons name="account-multiple" size={48} color={subTextColor} />
              <Text style={[styles.emptyTitle, { color: textColor }]}>No Friends Yet</Text>
              <Text style={[styles.emptySubtitle, { color: subTextColor }]}>Invite friends to see their progress</Text>
            </View>
          ) : (
            friends.map((friend) => (
              <FriendRow key={friend.id} friend={friend} onPress={() => navigation.navigate("FriendProgress", { friendId: friend.id })} darkMode={darkMode} />
            ))
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  addButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: "#F0FDFD", justifyContent: "center", alignItems: "center" },
  content: { padding: 16 },
