...
function createDayRecords(duration: number, startDate: Date): { date: string; status: DayStatus }[] {
  const today = new Date()
  const days: { date: string; status: DayStatus }[] = []
  for (let i = 0; i < duration; i++) {
    const date = addDays(startDate, i)
    const isPast = date < today && formatDateYYYYMMDD(date) !== formatDateYYYYMMDD(today)
