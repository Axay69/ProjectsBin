...
export default function InstaStoryViewerScreen({ route, navigation }: Props) {
  ...
  const durationMs = current.type === 'image' ? 10000 : Math.min(videoDurationMs || 0, 60000)
  ...
  const cacheVideo = async (uri: string) => {
    ...
    try {
      ...
      const res = await RNFetchBlob.config({ fileCache: true, appendExt: extGuess }).fetch('GET', uri)
      const path = res.path()
  ...
  const preloadAround = (center: number) => {
    ...
    const images = ahead.filter(i => i.type === 'image').map(i => ({ uri: i.uri }))
  ...
  return (
    <GestureHandlerRootView style={styles.container}>
      ...
      <View style={styles.content}>
        {current.type === 'image' ? (
          <FastImage source={{ uri: resolveUri(current) }} style={styles.media} resizeMode={FastImage.resizeMode.contain} />
        ) : (
          <Video
            ref={videoRef}
            ...
            controls={false}
            resizeMode="contain"
            onLoad={(data) => setVideoDurationMs(Math.min(data.duration * 1000, 60000))}
            onProgress={(data) => {
              const d = durationMs || Math.min((data.seekableDuration || data.playableDuration || 0) * 1000, 60000)
        ...
        {items.map((_, i) => {
          ...
          return (
            <View key={i} style={styles.progressSegment}>
              <View style={[styles.progressFill, { width: `${Math.max(0, Math.min(1, p)) * 100}%` }]} />
            </View>
          )
        })}
      </View>
      ...
      {(() => {
        const tap = Gesture.Tap()
          .onEnd(e => {
            if (e.x < width / 2) runOnJS(prev)(); else runOnJS(next)()
          })
        ...
        const pan = Gesture.Pan()
          .onEnd(e => { if (e.translationY > 100) runOnJS(navigation.goBack)() })
...
const styles = StyleSheet.create({
  ...
  closeIconBtn: { position: 'absolute', top: (StatusBar.currentHeight || 0), right: 10, width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', zIndex: 10 },
  closeIconText: { fontSize: 22, fontWeight: '700' },
  progressRow: { position: 'absolute', top: 12, left: 12, right: 12, flexDirection: 'row', gap: 6 },
  progressSegment: { flex: 1, height: 4, backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: 999, overflow: 'hidden' },
