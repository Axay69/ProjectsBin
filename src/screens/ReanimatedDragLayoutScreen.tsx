...
export default function ReanimatedDragLayoutScreen() {
  ...
  const reorder = (yValue: number) => {
    ...
    if (yValue > 0 && dragId) {
      ...
      newItems = [
        ...items.slice(0, indexOfDragId),
        ...(isLastIndex ? [] : [items[indexOfDragId + 1]]),
        items[indexOfDragId],
        ...(isLastIndex ? [] : items.slice(indexOfDragId + 2)),
      ];
    } else if (dragId) {
      const indexOfDragId = items.indexOf(dragId);
      ...
      newItems = isFirstIndex
        ? items
        : [
            ...items.slice(0, indexOfDragId - 1),
            items[indexOfDragId],
            items[indexOfDragId - 1],
            ...items.slice(indexOfDragId + 1),
          ];
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.desc}>
        Drag any item; others animate via layout()
      </Text>
