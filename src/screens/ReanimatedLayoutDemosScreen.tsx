...
type Props = NativeStackScreenProps<RootStackParamList, 'ReanimatedLayoutDemos'>
...
export default function ReanimatedLayoutDemosScreen(_: Props) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.desc}>Three focused demos that run entirely on the UI thread: Enter/Exit, List add/remove, and automatic Layout transitions.</Text>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>1) Beginner — Entering / Exiting</Text>
        <Text style={styles.cardDesc}>Avoid “pop-in” by animating mount and unmount. Uses `entering` and `exiting` props.</Text>
        <View style={styles.row}>
          <TouchableOpacity accessibilityRole="button" activeOpacity={0.7} onPress={() => setShowBox((s) => !s)} style={[styles.btn, styles.btnBlue]}>
            <Text style={styles.btnLabel}>{showBox ? 'Hide' : 'Show'} Box</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.boxArea}>
          {showBox && (
            <Animated.View entering={FadeIn.duration(250)} exiting={FadeOut.duration(250)} style={styles.demoBox} />
          )}
        </View>
      </View>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>2) Practical — Animated list add/remove</Text>
        <Text style={styles.cardDesc}>Smooth insertions and deletions. Each item defines `entering`, `exiting` and `layout` for reordering.</Text>
        <View style={styles.row}>
        ...
        <View style={styles.list}>
          {items.map((id) => (
            <Animated.View key={id} entering={FadeIn} exiting={FadeOut} layout={Layout.springify()} style={styles.listItem}>
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>3) Intermediate — Layout transitions</Text>
        <Text style={styles.cardDesc}>Animate position and size changes automatically with `layout={Layout.springify()}`.</Text>
...
const styles = StyleSheet.create({
  ...
  card: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 12, padding: 12, marginBottom: 12, backgroundColor: '#fff', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 6 },
  ...
  demoBox: { width: 60, height: 60, borderRadius: 12, backgroundColor: '#0EA5A4' },
  list: { gap: 8 },
  listItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 10, borderWidth: 1, borderColor: '#e5e7eb' },
  ...
  expander: { width: '100%', backgroundColor: '#fff', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 12, padding: 12 },
